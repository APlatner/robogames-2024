...
func _ready() -> void:
	...
	$EngineFullSpeedAudio.play()
	$EngineFullSpeedAudio.volume_db = -80

func _process(delta: float) -> void:
	var linear_input = Input.get_axis('backward', 'forward')
	var angular_input = -Input.get_axis('left', 'right')
	var rev_up_length = $EngineRevUpAudio.stream.get_length()
	var rev_down_length = $EngineRevDownAudio.stream.get_length()
	if linear_input == 0.0 and angular_input == 0.0 and _rev_down_anim == 0.0:
		_rev_up_anim = 0.0
		$EngineRevDownAudio.play()
		$EngineRevUpAudio.stop()
		$EngineFullSpeedAudio.volume_db = -80
		_rev_down_anim += delta / rev_down_length
	elif linear_input == 0.0 and angular_input == 0.0 and _rev_down_anim != 0.0:
		_rev_down_anim = clampf(_rev_down_anim, 0.0, 1.0)
		$EngineIdleAudio.volume_db = -80 + 80 * _rev_down_anim
		_rev_down_anim += delta / rev_down_length
	elif (linear_input != 0.0 or angular_input != 0.0) and _rev_up_anim == 0.0:
		_rev_down_anim = 0.0
		$EngineRevUpAudio.play()
		$EngineRevDownAudio.stop()
		$EngineIdleAudio.volume_db = -80
		_rev_up_anim += delta / rev_up_length
	elif (linear_input != 0.0 or angular_input != 0.0) and _rev_up_anim != 0.0:
		_rev_up_anim = clampf(_rev_up_anim, 0.0, 1.0)
		$EngineFullSpeedAudio.volume_db = -80 + 80 * _rev_up_anim
		_rev_up_anim += delta / rev_up_length
